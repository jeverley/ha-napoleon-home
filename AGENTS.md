...
- Poll: send `Gpr {"n": "<property>", "t": <type_code>}` → grill replies with `gpr {"n": ..., "v": ..., "t": <type>}` (no ACK needed). The `t` field in the request is required for some properties (e.g. `DTYPE`, `GS_UNT`, `REGN`, `battery_low_alert`) — omitting it gets a rejection with no `v` field and a status code instead (`s:1` general failure or `s:7` required field missing), even though most properties tolerate its absence
...
- Ayla `s` status codes (present instead of `p`/`v` on a rejected request): `1`=general failure, `3`=invalid command, `4`=permanent auth failure, `6`=not provisioned, `7`=required field missing
